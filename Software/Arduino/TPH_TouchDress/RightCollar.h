...
class RightCollar: public Gesture{
public:

    RightCollar(LightManager* lightManager, int touchPin);
...
RightCollar::RightCollar(LightManager* lightManager, int touchPin): Gesture(lightManager,touchPin)
{
  _pin           = 0;
 ...
 void RightCollar::update(bool currState, int _touchPin)
 {   
     ...
     if(!currState){
       lightManager->setTouchFading(false);
     }
 ...
 void RightCollar::updateGesture(bool currState)
 {
  ...
  void RightCollar::printGesture()
 {
     // Save click codes in LEDfunction, as click codes are reset at next Update()
      if (clicks != 0) function = clicks;
    
      if(clicks == 1) Serial.println("RightCollar::SINGLE click");
    
      if(function == 2) {
            Serial.println("RightCollar::DOUBLE click");
            //lightManager->toggleOnOff();
            lightManager->setFading(true);
      }
    
      if(function == 3) Serial.println("TRightCollar::RIPLE click");
    
      if(function == -1)
      {
          lightManager->setTouchFading(true);
          Serial.println("RightCollar::SINGLE LONG click");
      }
    
      if(function == -2) Serial.println("RightCollar::DOUBLE LONG click");
    
      if(function == -3) Serial.println("RightCollar::TRIPLE LONG click");
